...
def process_rose_data(windSpeeds,windDirections,windFrequencies,nDirections,nSpeeds):
    ...
    dir_int1 = np.linspace(dirs[0],dirs[0]+ddir/2.,int(num_int/2))
    freq_int1 = np.zeros(int(num_int/2))
    speed_freq_int1 = np.zeros(int(num_int/2))
    for j in range(int(num_int/2)):
        freq_int1[j],_,_,_ = spline_freq.interp(dir_int1[j])
        ws,_,_,_ = spline_speed.interp(dir_int1[j])
        speed_freq_int1[j] = freq_int1[j]*ws

    dir_int2 = np.linspace(dirs[0],dirs[0]+ddir/2.,int(num_int/2))
    freq_int2 = np.zeros(int(num_int/2))
    speed_freq_int2 = np.zeros(int(num_int/2))
    for j in range(int(num_int/2)):
