...
for i in range(nruns):
    filename = "/glb/hou/pt.sgs/data/offshorewind/uspsty/Projects/GeometricYaw/example_optimizations/2_results/results_%s.yml"%i
    with open(filename, 'r') as stream:
        data_loaded = yaml.safe_load(stream)
    layout_calls[i] = data_loaded["function_calls"]
    layout_time[i] = data_loaded["run_time"]

    filename = "/glb/hou/pt.sgs/data/offshorewind/uspsty/Projects/GeometricYaw/example_optimizations/2_results_codesign/results_codesign_%s.yml"%i
    with open(filename, 'r') as stream:
        data_loaded = yaml.safe_load(stream)
    codesign_calls[i] = data_loaded["function_calls"]
    codesign_time[i] = data_loaded["run_time"]
