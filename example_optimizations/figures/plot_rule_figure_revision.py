...
for path, currentDirectory, files in os.walk("/glb/hou/pt.sgs/data/offshorewind/uspsty/Projects/GeometricYaw/yaw_data_grid"):
    for file in files:
...
for path, currentDirectory, files in os.walk("/glb/hou/pt.sgs/data/offshorewind/uspsty/Projects/GeometricYaw/yaw_data"):
    for file in files:
        filename = os.path.join(path, file)
        data = np.load(filename)
        yaw_angles = np.append(yaw_angles, data["yaw_angles"])
...
plt.yticks(fontsize=8)
print(len(x_array))
ax3.scatter(x_array,y_array,s=2.0,c=yaw_angles,cmap=mycolormap,vmin=-30.0,vmax=30.0)
ax3.set_xlim(4,35)
...
plt.savefig("yaw_rule_revision.png",transparent=True)
